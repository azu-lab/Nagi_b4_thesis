/*
 * This file was automatically generated by tecsgen.
 * This file is not intended to be edited.
 */
#ifndef tSingle_TECSGEN_H
#define tSingle_TECSGEN_H

/*
 * celltype          :  tSingle
 * global name       :  tSingle
 * multi-domain      :  no
 * idx_is_id(actual) :  no(no)
 * singleton         :  no
 * has_CB            :  no
 * has_INIB          :  no
 * rom               :  yes
 * CB initializer    :  no
 */

/* global header #_IGH_# */
#include "global_tecsgen.h"

/* signature header #_ISH_# */
#include "sMulti_tecsgen.h"

#ifndef TOPPERS_MACRO_ONLY

#ifdef __cplusplus
extern "C" {
#endif /* __cplusplus */
/* cell CB (dummy) type definition #_CCDP_# */
typedef struct tag_tSingle_CB {
    int  dummy;
} tSingle_CB;
/* singleton cell CB prototype declaration #_MCPB_# */

/* celltype IDX type #_CTIX_# */
typedef int   tSingle_IDX;
#ifdef __cplusplus
}
#endif /* __cplusplus */
#endif /* TOPPERS_MACRO_ONLY */

/* to get the definition of CB type of referenced celltype for optimization #_ICT_# */
#ifndef  TOPPERS_CB_TYPE_ONLY
#define  tSingle_CB_TYPE_ONLY
#define TOPPERS_CB_TYPE_ONLY
#endif  /* TOPPERS_CB_TYPE_ONLY */
#include "tMulti_tecsgen.h"
#ifdef  tSingle_CB_TYPE_ONLY
#undef TOPPERS_CB_TYPE_ONLY
#endif /* tSingle_CB_TYPE_ONLY */
#ifndef TOPPERS_CB_TYPE_ONLY

#define tSingle_ID_BASE             (1)  /* ID Base  #_NIDB_# */
#define tSingle_N_CELL              (1)  /*  number of cells  #_NCEL_# */

/* IDX validation macro #_CVI_# */
#define tSingle_VALID_IDX(IDX) (1)


/* celll CB macro #_GCB_# */
#define tSingle_GET_CELLCB(idx) ((void *)0)
#ifndef TECSFLOW
 /* call port function macro #_CPM_# */
#define tSingle_cMulti1_print( p_that ) \
	  tMulti_eMulti_print( \
	   (tMulti_IDX)0 )
#define tSingle_cMulti2_print( p_that ) \
	  tMulti_eMulti_print( \
	   (tMulti_IDX)0 )

#else  /* TECSFLOW */
#define tSingle_cMulti1_print( p_that ) \
	  (p_that)->cMulti1.print__T( \
 )
#define tSingle_cMulti2_print( p_that ) \
	  (p_that)->cMulti2.print__T( \
 )

#endif /* TECSFLOW */
#endif /* TOPPERS_CB_TYPE_ONLY */

#ifndef TOPPERS_MACRO_ONLY

#ifdef __cplusplus
extern "C" {
#endif /* __cplusplus */

#ifndef TOPPERS_CB_TYPE_ONLY

#endif /* TOPPERS_CB_TYPE_ONLY */

#ifdef __cplusplus
}
#endif /* __cplusplus */
#endif /* TOPPERS_MACRO_ONLY */

#ifndef TOPPERS_CB_TYPE_ONLY

/* IDX validation macro (abbrev.) #_CVIA_# */
#define VALID_IDX(IDX)  tSingle_VALID_IDX(IDX)


/* cell CB macro (abbrev) #_GCBA_# */
#define GET_CELLCB(idx)  tSingle_GET_CELLCB(idx)

/* CELLCB type (abbrev) #_CCT_# */
#define CELLCB	tSingle_CB

/* celltype IDX type (abbrev) #_CTIXA_# */
#define CELLIDX	tSingle_IDX

/* call port function macro (abbrev) #_CPMA_# */
#define cMulti1_print( ) \
          ((void)p_cellcb, tSingle_cMulti1_print( p_cellcb ))
#define cMulti2_print( ) \
          ((void)p_cellcb, tSingle_cMulti2_print( p_cellcb ))



/* iteration code (FOREACH_CELL) (niether CB, nor NIB exit) #_NFEC_# */
#define FOREACH_CELL(i,p_cb)   \
    for((i)=0;(i)<0;(i)++){

#define END_FOREACH_CELL   }

/* CB initialize macro #_CIM_# */
#endif /* TOPPERS_CB_TYPE_ONLY */

#ifndef TOPPERS_MACRO_ONLY

#endif /* TOPPERS_MACRO_ONLY */

#endif /* tSingle_TECSGENH */
